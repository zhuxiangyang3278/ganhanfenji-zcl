...
class Evaluator():
    ...
    def input_check(cls, imgF, imgA=None, imgB=None):
        if imgA is None:
            ...
            assert isinstance(imgF, np.ndarray) and isinstance(imgA, np.ndarray) and isinstance(
                imgB, np.ndarray), 'type error'
    ...
    def compare_viff(cls, ref, dist):  # viff of a pair of pictures
        ...
        for scale in range(1, 5):
            ...
            mu2_sq = mu2 * mu2
            mu1_mu2 = mu1 * mu2
            sigma1_sq = convolve2d(
                ref * ref, np.rot90(win, 2), mode='valid') - mu1_sq
            sigma2_sq = convolve2d(
                dist * dist, np.rot90(win, 2), mode='valid') - mu2_sq
            sigma12 = convolve2d(
                ref * dist, np.rot90(win, 2), mode='valid') - mu1_mu2
    ...
    def Qabf_getQabf(cls, aA, gA, aF, gF):
        ...
        ka = -22
        Da = 0.8
        GAF, AAF, QgAF, QaAF, QAF = np.zeros_like(aA), np.zeros_like(
            aA), np.zeros_like(aA), np.zeros_like(aA), np.zeros_like(aA)
    ...
    def SSIM(cls, image_F, image_A, image_B):
        ...
        data_range = 255 if image_F.max() > 1 else 1.0
...
def VIFF(image_F, image_A, image_B):
    ...
    for scale in range(1, 5):
        ...
        mu1A_mu2 = mu1A * mu2
        mu1B_mu2 = mu1B * mu2
        sigma1A_sq = convolve2d(
            refA * refA, np.rot90(win, 2), mode='valid') - mu1_sq_A
        sigma1B_sq = convolve2d(
            refB * refB, np.rot90(win, 2), mode='valid') - mu1_sq_B
        sigma2_sq = convolve2d(
            dist * dist, np.rot90(win, 2), mode='valid') - mu2_sq
        sigma12_A = convolve2d(
            refA * dist, np.rot90(win, 2), mode='valid') - mu1A_mu2
        sigma12_B = convolve2d(
            refB * dist, np.rot90(win, 2), mode='valid') - mu1B_mu2
        ...
        numA += np.sum(np.log10(1 + gA * gA *
                       sigma1A_sq / (sv_sq_A + sigma_nsq)))
        numB += np.sum(np.log10(1 + gB * gB *
                       sigma1B_sq / (sv_sq_B + sigma_nsq)))
